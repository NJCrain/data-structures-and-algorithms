...
public class StackTest {
    ...
    public void testPush() {
        ...
        assertEquals("The top should now point to the new node", 5, test.top.value);
        assertNull("The new Node's next should be null", test.top.next);

        test.push(4);

        assertEquals("top should now point to the newer node", 4, test.top.value);
        assertEquals("The next node should be the one we added previously", 5, test.top.next.value);
    }

    @Test
    public void testPop() {
        assertEquals("Popping an empty list should give back a 0", 0, test.pop());

        test.push(5);
        test.push(4);

        assertEquals("The first pop should give back the last value added", 4, test.pop());
        assertEquals("Calling pop again should return the value added before 4", 5, test.pop());
    }
